        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7">
            ...
            <p className="mt-5 text-lg text-neutral-600 max-w-2xl">
              I help B2B teams ship the right thing faster: product strategy, lean UX, and practical automation.
              Friendly to work with. Brutally honest with the work.
            </p>
...
type LogoBoxProps = { text: string };
function LogoBox({ text }: LogoBoxProps) {
  return (
...
type ServiceProps = { title: string; points: string[] };
function Service({ title, points }: ServiceProps) {
  return (
...
type CaseProps = { title: string; result: string; tag: string };
function Case({ title, result, tag }: CaseProps) {
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      ...
      <div className="mt-3 rounded-lg bg-neutral-50 p-3 text-sm">
        Outcome: <span className="font-semibold">{result}</span>
      </div>
    </div>
  );
}

type StepProps = { n: number; title: string; body: string };
  ...
  return (
    <div className="mt-12 rounded-2xl border border-neutral-200 bg-white p-6">
      <div className="flex items-start gap-4">
        <BotGlyph small />
        <blockquote className="text-neutral-700 text-sm md:text-base">
          “Nicole takes complex messes and returns simple, actionable plans. No fluff, just movement.”
          <span className="text-neutral-500"> — Product VP, SaaS</span>
        </blockquote>
      </div>
    </div>
  );
...
type MetricProps = { title: string; value: string };
function HeroVisual() {
  return (
